(()=>{
  ...
  style.textContent=`
    ...
    @media(max-width:520px){.custom-form{grid-template-columns:1fr}.custom-form .wide{grid-column:auto}.custom-item{grid-template-columns:64px 1fr}.custom-thumb{width:64px;height:64px}}
  `;
  document.head.appendChild(style);

  const section=document.createElement('section');section.id='customWords';
  section.innerHTML=`<h2>じぶんのことば</h2><p class="desc">好きな写真とことばを追加できます。表示する名前と、実際に読み上げることばは別々にできます。</p><div class="custom-form"><input id="customLabel" type="text" maxlength="20" placeholder="表示する名前（例：おばあちゃん）"><input id="customSpeak" type="text" maxlength="40" placeholder="読み上げることば（空なら同じ）"><button id="customPhoto" type="button">🖼️ 写真をえらぶ</button><button id="customAdd" class="add" type="button">＋ 追加</button><div id="customPhotoName" class="wide custom-status">写真なしでも追加できます</div></div><div id="customList"></div>`;
  grid.parentNode.insertBefore(section,grid);

  const labelInput=section.querySelector('#customLabel'),speakInput=section.querySelector('#customSpeak'),photoBtn=section.querySelector('#customPhoto'),addBtn=section.querySelector('#customAdd'),photoName=section.querySelector('#customPhotoName'),list=section.querySelector('#customList');
  const newPhotoInput=document.createElement('input');newPhotoInput.type='file';newPhotoInput.accept='image/*';newPhotoInput.hidden=true;document.body.appendChild(newPhotoInput);let pendingPhoto=null;
  ...
  newPhotoInput.addEventListener('change',async()=>{const f=newPhotoInput.files?.[0];newPhotoInput.value='';if(!f)return;try{pendingPhoto=await resizeSquare(f);photoName.textContent='✓ 写真を選びました'}catch(e){alert('写真を読み込めませんでした')}});
  addBtn.addEventListener('click',async()=>{const label=labelInput.value.trim(),speak=(speakInput.value.trim()||label);if(!label){alert('表示する名前を入力してください');return}const id=(crypto.randomUUID?.()||Date.now().toString(36)+Math.random().toString(36).slice(2));await saveItem({id,label,speak,photo:pendingPhoto||null,createdAt:Date.now()});labelInput.value='';speakInput.value='';pendingPhoto=null;photoName.textContent='写真なしでも追加できます';await render()});
