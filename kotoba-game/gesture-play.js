(()=>{
  ...
  style.textContent=`
    .toy.gesture-held{animation-play-state:paused!important;transition:transform .08s linear,filter .15s ease;filter:drop-shadow(0 14px 12px rgba(0,0,0,.2)) brightness(1.03);z-index:18}
  ...
  function startToy(e,toy){
    ...
    toyGesture={id:e.pointerId,toy,startX:e.clientX,startY:e.clientY,dx:0,dy:0,long:false};
    toy.classList.add('gesture-held');toy.style.transform='scale(1.12)';
    try{toy.setPointerCapture(e.pointerId)}catch(err){}
    clearTimeout(holdTimer);
    holdTimer=setTimeout(()=>{
      if(!toyGesture||toyGesture.toy!==toy)return;
      toyGesture.long=true;
      toy.style.transform=`translate(${toyGesture.dx}px,${toyGesture.dy}px) scale(1.26)`;
  ...
  function moveToy(e){
    ...
    e.preventDefault();
    const dx=Math.max(-95,Math.min(95,e.clientX-toyGesture.startX)),dy=Math.max(-95,Math.min(95,e.clientY-toyGesture.startY));
    ...
    toyGesture.toy.style.transform=`translate(${dx}px,${dy}px) scale(${scale})`;
  }
  function finishToy(e,fire=true){
    ...
    toy.classList.remove('gesture-held');toy.style.transition='transform .2s cubic-bezier(.2,.8,.2,1.2)';toy.style.transform='translate(0,0) scale(1)';
    setTimeout(()=>{if(toy.isConnected){toy.style.transition='';toy.style.transform=''}},220);
    toyGesture=null;
    if(!fire||!toy.isConnected)return;
    // Let the original game handler run once, on release. Synthetic pointerdown
    // is ignored by this capture handler (isTrusted=false), so it reaches the toy.
